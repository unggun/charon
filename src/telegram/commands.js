...
export async function handleMessage(msg) {
  ...
  if (text.startsWith('/stratset')) {
    ...
    if (!id || !key || !value) {
      return bot.sendMessage(chatId, 'Usage: /stratset <strategy_id> <key> <value>\n\nExample: /stratset sniper tp_percent 75\n\nKeys: tp_percent, sl_percent, position_size_sol, max_open_positions, min_mcap_usd, max_mcap_usd, min_holders, trailing_enabled, trailing_percent, trailing_arm_at_percent, partial_tp, partial_tp_at_percent, partial_tp_sell_percent, max_hold_ms, use_llm, llm_min_confidence, min_source_count, require_fee_claim, min_fee_claim_sol, min_gmgn_total_fee_sol, max_ath_distance_pct, min_fee_density_sol_per_hour, early_stage_mcap_threshold_usd, early_llm_min_confidence, early_min_fee_claim_sol, early_min_gmgn_total_fee_sol, early_max_ath_distance_pct, early_min_holders, early_max_top20_holder_percent, min_trench_score, min_organic_score, skip_bonding_band_min, skip_bonding_band_max, rug_guard_drop_pct, max_jup_bundler_ath_pct, max_change5m_pct');
    }
    const strat = strategyById(id);
    if (!strat) return bot.sendMessage(chatId, `Strategy "${id}" not found.`);
    const numKeys = new Set(['tp_percent', 'sl_percent', 'position_size_sol', 'max_open_positions', 'min_mcap_usd', 'max_mcap_usd', 'min_holders', 'max_top20_holder_percent', 'trailing_percent', 'trailing_arm_at_percent', 'partial_tp_at_percent', 'partial_tp_sell_percent', 'max_hold_ms', 'llm_min_confidence', 'min_source_count', 'min_fee_claim_sol', 'min_gmgn_total_fee_sol', 'max_ath_distance_pct', 'token_age_max_ms', 'trending_min_volume_usd', 'trending_min_swaps', 'trending_max_rug_ratio', 'trending_max_bundler_rate', 'min_saved_wallet_holders', 'min_graduated_volume_usd', 'min_fee_density_sol_per_hour', 'early_stage_mcap_threshold_usd', 'early_llm_min_confidence', 'early_min_fee_claim_sol', 'early_min_gmgn_total_fee_sol', 'early_max_ath_distance_pct', 'early_min_holders', 'early_max_top20_holder_percent', 'min_trench_score', 'min_organic_score', 'skip_bonding_band_min', 'skip_bonding_band_max', 'rug_guard_drop_pct', 'max_jup_bundler_ath_pct', 'max_change5m_pct']);
